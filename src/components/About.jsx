import React from 'react'

function About() {
  return (

    <div name="About" className='max-w-screen-2xl container ms-auto px-4 md:px-20 my-16'>
     <div>
     <h1 className='text-3xl font-bold mb-5'>About</h1>
      <p>I'm a skilled software developer with experience in JavaScript, and expertise in frameworks like React, Springboot along with MongoDB. I'm a quick learner and collaborate closely with clients to create efficient, scalable, and user-friendly solutions that solve real-world problems. Let's work together to bring your ideas to life!</p>
      <br />
      <h1 className='text-green-600 font-semibold text-xl'>Education & Training </h1>
      <span>
        [Degree/certification].institution, year Lorem ipsum dolor sit amet Lorem ipsum dolor sit, amet consectetur adipisicing elit. Culpa!
      </span>
      <br />
      <br />
      <h1 className='text-green-600 font-semibold text-xl'>Skill & Experience </h1>
      <span>
        [Degree/certification].institution, year Lorem ipsum dolor sit amet Lorem ipsum dolor sit, amet consectetur adipisicing elit. Culpa!
      </span>
      <br />
      <br />
      <h1 className='text-green-600 font-semibold text-xl'> Mission Statement </h1>
      <span>
        [Degree/certification].institution, year Lorem ipsum dolor sit amet Lorem ipsum dolor sit, amet consectetur adipisicing elit. Culpa!
      </span>
     </div>
    </div>
  )
}

export default About
